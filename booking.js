...
async function loadBookings() {
    ...
    try {
        const response = await fetch(apiUrl(`/api/clientBookings?${buildIdentityQuery()}`));
        if (response.ok) {
...
function renderBookings(list) {
            ...
            document.querySelectorAll(".cancelBtn").forEach(btn => {
                btn.onclick = async (e) => {
                    ...
                    showDeleteBookingPopup("Are you sure you want to cancel this booking?", async () => {
                        try {
                            const response = await fetch(apiUrl(`/api/book/${bookingId}?${identityQuery}`), {
                                method: 'DELETE'
                            });
    ...
    async function startEdit(bookingId) {
        ...
        try {
            const response = await fetch(apiUrl(`/api/book/${bookingId}`));
...
function cancelEdit() {
    ...
    async function updateBooking() {
        ...
        showBookingPopup(`Update booking with new details?`, async () => {
            try {
                const response = await fetch(apiUrl(`/api/book/${editingBookingId}`), {
                    method: 'PUT',
...
if (bookBtn) {
    ...
    async function loadMyFeedbacks() {
        ...
        try {
            const response = await fetch(apiUrl(`/api/feedback/my-feedbacks?${buildIdentityQuery()}`));
            ...
            myFeedbacksEl.querySelectorAll(".feedback-remove-btn").forEach(btn => {
                btn.onclick = () => {
                    showDeleteFeedbackPopup("Are you sure you want to remove this feedback?", async () => {
                        try {
                            const response = await fetch(apiUrl(`/api/feedback/${btn.dataset.id}?${buildIdentityQuery()}`), {
                                method: 'DELETE'
                            });
                            const result = await response.json();
